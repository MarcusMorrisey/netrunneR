...
| `cobra.sql` | Ten tables (`tournament`, `stage`, `round`, `pairing`, `standing`, `faction_count`, `identity_count`, `cut_conversion_faction`, `cut_conversion_identity`, `recent_index`), columns exactly matching the `COBRA_*_ALLOWLIST` constants | Adding a Cobra column or table, changing Cobra's schema |
