...
render_board <- function(add_ice = character(0), add_breaker = character(0),
  ...
  shiny::testServer(
    ...
    {
      for (code in add_ice) session$getReturned()$add_ice(code)
      for (code in add_breaker) session$getReturned()$add_breaker(code)
      session$flushReact()
...
test_that("a hand-curated pair is badged OVERRIDE, not passed off as derived", {
  ...
  rendered <- render_board(add_ice = "ice01", add_breaker = "brk01")
...
test_that("a runner-favored differential reads as positive and a corp-favored one as negative", {
  runner_favored <- render_board(add_ice = "ice01", add_breaker = "brk01")
  expect_match(runner_favored, "nr-diff-good")
  expect_match(runner_favored, "[+]1")

  corp_favored <- render_board(add_ice = "ice02", add_breaker = "brk02")
...
test_that("an uncomputable pair says so instead of showing a blank strip", {
  ...
  rendered <- render_board(add_ice = "ice03", add_breaker = "brk03")
...
test_that("the same breaker set is applied to every lane, so strips line up", {
  rendered <- render_board(add_ice = c("ice01", "ice02"), add_breaker = "brk01")
