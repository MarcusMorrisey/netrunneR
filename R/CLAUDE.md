...
| `build-cobra.R` | `COBRA_*_ALLOWLIST` constants, `COBRA_DENY_PATTERN`, `flatten_cobra_*()`, `build_cobra()` (also wires in the abr/cobra merge, `merge-abr-cobra.R`) | Changing Cobra's ten tables, allowlists, bundle-flattening logic, or how the merge is wired in |
| `abr-cobra-xref.R` | Human-verified abr/cobra override log readers (`read_abr_cobra_verified_matches()`, `_deletes()`, `_group_matches()`) and their fail-closed validators | Editing the override CSVs or their loader/validation contract |
| `merge-abr-cobra.R` | `merge_abr_cobra()`'s four-tier override precedence, `merge_survivorship()` per-column rules, `cobra_tournament_facts()` player-count/winner derivation, `resolve_cobra_identity_code()`, `abr_cobra_algorithmic_matches()` | Changing the abr/cobra merge, its survivorship rules, or the algorithmic matching tier |
...
| `operations.R` | `alert_box()`, `safe_render()`, `click_sets_input()`, `resolve_active_release()`/`query_active_release()` (also used by `build-implementation.R`'s `cardpool_codes()`), `read_matchup_overrides()`, `read_active_release_tables()`/`CARDPOOL_LEGALITY_TABLES`, `select_tournaments_source()` (cobra's merged feed over abr's own table over `NULL`), `load_ice_breaker_app_data()` -- shared Shiny-app plumbing: styled alerts, render-error fallback, click-to-input wiring, and the once-per-process cardpool/implementation/matchup/tournament load `inst/shiny-app/app.R` calls | Adding a new primary render that should degrade gracefully, a new clickable element, or changing how the app loads its data or picks its tournament source |
