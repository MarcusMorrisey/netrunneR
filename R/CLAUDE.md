...
| `operations.R` | `safe_render()` -- catches a Shiny output's render error instead of letting Shiny's default per-output error banner surface it | Adding a new primary render that should degrade gracefully |
