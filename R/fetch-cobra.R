#' Fetch Cobra tournament data (NSG's official tournament platform)
#'
#' Fetches from https://tournaments.nullsignal.games/ (internally "Cobra
#' tournament data"), a source with no complete historical index endpoint
#' -- only 12 public "type" listing pages (recent tournaments per type)
#' and per-tournament JSON endpoints. Unlike fetch_abr(), which learns its
#' complete id set from one paginated API call, fetch_cobra() combines
#' three id-discovery mechanisms every time it runs, regardless of mode:
#' (1) scraping the 12 type pages for currently-listed ids, (2) probing a
#' short tail of ids above the highest id ever seen for newly created
#' tournaments, and (3) advancing a bounded batch of a persistent
#' low-to-high id walk to gradually backfill tournaments no longer listed
#' on any type page. This is a deliberate deviation from the abr/nrdb
#' pattern: mode ("scheduled"/"backfill"/"rollback") never reaches
#' fetch_lineage() in this codebase (see run_sync(), R/sync.R -- mode
#' only gates no_op_change()'s short-circuit and rollback dispatch), so
#' fetch_cobra() cannot branch on mode the way sync_nullsignal_tournaments.R's
#' standalone --mode flag did. Instead the historical walk advances by a
#' fixed batch on every fetch call, scheduled or backfill alike -- the
#' same "cheap to resume, mode-independent" idiom fetch_abr() already
#' uses for run_abr_backfill().
#'
...
#' always refreshed (their pairings/standings can change while an event is
#' live); tournaments found only via the historical batch walk are fetched
#' once and treated as immutable thereafter. Every sync attempt rebuilds
#' its tables from the *entire* pool, not just the ids touched this run,
#' so coverage grows monotonically across runs.
#'
...
fetch_cobra <- function(lineage, attempt_dir) {
  ...
  jsonlite::write_json(recent_index, file.path(raw_dir, "recent_index.json"), auto_unbox = TRUE)
...
#' unauthenticated crawl this lineage runs (same convention
#' sync_nullsignal_tournaments.R's perform_request() used). A 429 or 5xx
#' is retried with capped exponential backoff up to max_retries; a
#' 5xx exhausting retries aborts the whole attempt, same fail-closed
#' default as abr_get()'s hard stop.
#'
...
cobra_get <- function(lineage, path, query = list(), as = c("json", "html"), max_retries = 4L) {
  ...
  for (attempt in seq_len(max_retries)) {
    ...
    if (status >= 200 && status < 300) {
      ...
      # (R/build-cobra.R) walk with $ access and for-loops, matching
      # sync_nullsignal_tournaments.R's identical choice for the same
      # reason -- unlike abr_get()/nrdb_get()'s simplifyVector = TRUE,
      # which suits their flat, uniform arrays-of-objects instead.
    ...
    rlang::abort(
      ...
      class = "netrunneR_cobra_http_error"
    )
  }

  rlang::abort(sprintf("Cobra request exhausted retries for %s", path), class = "netrunneR_cobra_retry_exhausted")
}
...
#'
#' Stops once COBRA_TAIL_MISS_LIMIT consecutive ids do not exist, matching
#' sync_nullsignal_tournaments.R's probe_new_tail() -- a bounded,
#' per-run cost independent of how far the true upper bound has moved.
#' @keywords internal
...
#'
#' Mirrors sync_nullsignal_tournaments.R's scrape_bundle(): the pairings
#' endpoint is authoritative for whether the tournament exists at all
#' (its own 404 means the id is unassigned); the other three endpoints
#' are fetched only once pairings confirms existence, and a 404 on any of
#' them is treated as "not published for this tournament" rather than
#' aborting the bundle.
#' @keywords internal
...
#' defaults if absent -- same shape as
#' sync_nullsignal_tournaments.R's load_state(), persisted as JSON rather
#' than RDS so it stays human-inspectable on disk.
#' @keywords internal
...
#' discovers a higher max_known_id resets that flag, same as
#' sync_nullsignal_tournaments.R's crawl state.
#' @keywords internal
